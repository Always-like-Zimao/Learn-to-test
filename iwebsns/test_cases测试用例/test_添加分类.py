...
# 点击添加分类
        self.key.click(By.XPATH,'//a[@onclick="newShow();return false;"]')
        sleep(1)
        self.key.input(By.XPATH,"//input[@id='new_sort_name']",'梓喵')
        self.key.click(By.XPATH,"//input[@value='保存']")
        sleep(1)

        try:
            self.assertIn('梓喵',self.driver.title)
            sleep(2)
            print("第七个测试用例失败")
            logger.info("验证失败，未进入正确的页面Failed")
